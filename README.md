...
```sh
export EUCSTRING_MODE=euc-jp # 移行前
./app

export EUCSTRING_MODE=utf-8 # UTF-8 DBへの切り替え後
./app
```
...
モード切り替え前に、DBと接続セッションのエンコーディングが一致していることを確認してください。
...
`server_encoding`、`client_encoding`、ドライバの返却形式がUTF-8になってから、`EUCSTRING_MODE=utf-8`へ切り替えます。
